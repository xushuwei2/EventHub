...
本设计的目标是新增一个名为 `EventHub` 的独立前端事件采集服务，用于统一接收、清洗、聚合多产品前端错误与行为数据，并在服务内提供带用户名密码登录的简易后台页面。
...
- 支持采集以下事件：
  ...
  - 资源加载失败
  - 关键业务异常
- 支持按错误指纹聚合入库，保留版本、语言、平台等维度字段。
- 支持多产品接入，按 `projectKey` 隔离数据。
- 提供最小可用后台页面，支持登录、项目配置、用户反馈、行为分析与漏斗转化。
